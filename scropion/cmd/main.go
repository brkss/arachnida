...
var exifTagNames = map[uint16]string{
	...
	0x013B: "Artist",
	0x013C: "WhiteBalance",
	0x013D: "DigitalZoomRatio",
	0x013E: "FocalLengthIn35mmFormat",
	0x013F: "SceneCaptureType",
}
...
func main() {
	...
	for offset < len(data) -2 {
		if data[offset] == 0xFF && data[offset + 2] == 0xE1 {
			
			segSize := int(data[offset + 2]) << 8 + int(data[offset + 3])
			start:= offset + 4 
			end := offset + segSize + 2
			if end > len(data){
				fmt.Println(">>> found app1 segement : invalid len ")
				break;
			}
